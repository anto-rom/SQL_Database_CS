...
CREATE TABLE APP_FLOW.CustomerService.tickets_sla ( tickets_sla_id int IDENTITY(1,1) NOT NULL, department_id int NOT NULL, [Date] date NOT NULL, sla_ticket decimal(18,2) NULL, priority_id int NULL, CONSTRAINT tickets_sla_pkey PRIMARY KEY (tickets_sla_id));
...
CREATE TABLE APP_FLOW.CustomerService.TL_discretion ( TL_discretion_id int IDENTITY(1,1) NOT NULL, tl_id int NOT NULL, agent_id int NOT NULL, discretion_score numeric(10,2) NULL, Month_number int NULL, CONSTRAINT PK__TL_discr__49A1F1F8CE9652BE PRIMARY KEY (TL_discretion_id), CONSTRAINT FK_TL_discretion_TL FOREIGN KEY (tl_id) REFERENCES APP_FLOW.CustomerService.tl(tl_id), CONSTRAINT FK_TL_discretion_agent FOREIGN KEY (agent_id) REFERENCES APP_FLOW.CustomerService.agent(agent_id));
...
outbounds AS (
    SELECT
        ...
        o.department_id,
        o.[Date],
        SUM(o.total_outbounds) AS total_outbounds
    FROM CustomerService.outbounds o
...
SELECT
    ...
    dp.department_name,  -- si esto diera conflicto en otras vistas, también puedes COLLATE aquí
    CONCAT(
...
SELECT
    ...
    s.shift_time,
    sh.shrinkage_time,
    (s.shift_time - sh.shrinkage_time) AS effective_time,
    p.productivity_total,
    ROUND(
        a.item_target * 1.0 * (s.shift_time - sh.shrinkage_time) / s.shift_time,
        2
    ) AS adjusted_target,
    ROUND(
        p.productivity_total * 1.0 /
        NULLIF(
            a.item_target * 1.0 * (s.shift_time - sh.shrinkage_time) / s.shift_time,
            0
        ) * 100,
...
FROM
    ...
    INNER JOIN CustomerService.shrinkage sh ON a.agent_id = sh.agent_id AND s.Date = sh.Date
    INNER JOIN CustomerService.productivity p ON a.agent_id = p.agent_id AND s.Date = p.Date
WHERE
