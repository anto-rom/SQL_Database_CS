...
CREATE TABLE APP_FLOW.CustomerService.resolution_sla ( resolution_sla_id int IDENTITY(1,1) NOT NULL, [Date] date NULL, department_id int NULL, case_number int NULL, agent_id int NULL, case_subreason varchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL, age int NULL, resolution_sla float NULL, closed_Date date NULL, CONSTRAINT resolution_sla_pkey PRIMARY KEY (resolution_sla_id));
...
AS
WITH case_tp_day AS (
    /* Count touchpoints per case per day */
    SELECT
        CAST(r.[Date] AS date) AS [date],
        r.case_number,
        COUNT(*) AS tp_cnt_day
    FROM [APP_FLOW].[CustomerService].[repeats] AS r
    GROUP BY CAST(r.[Date] AS date), r.case_number
),
flagged_cases AS (
    /* Cases that have >= 3 touchpoints on that specific day */
    SELECT
        ctd.[date],
        ctd.case_number
    FROM case_tp_day AS ctd
    WHERE ctd.tp_cnt_day >= 3
),
daily_rows AS (
    /* One row per touchpoint, flagging whether it belongs to a "repeat" case for that day */
    SELECT
        CAST(r.[Date] AS date) AS [date],
        r.department_id,
        r.agent_id,
    ...
    LEFT JOIN flagged_cases AS fc
           ON fc.case_number = r.case_number
          AND fc.[date]      = CAST(r.[Date] AS date)
)
SELECT
    dr.[date],
    dr.department_id,
...
LEFT JOIN [APP_FLOW].[CustomerService].[agent]      AS a
       ON a.agent_id = dr.agent_id
LEFT JOIN [APP_FLOW].[CustomerService].[department] AS d
...
GROUP BY GROUPING SETS (
    ...
    (dr.[date], dr.department_id, d.department_name, dr.account_country)                      --    (dr.[date], dr.department_id, d.department_name, dr.account_country)                      -- por país
);
...
-- CustomerService.agent_productivity_new_daily (modelo unificado Vonage / no-Vonage)
ALTER VIEW CustomerService.agent_productivity_new_daily
AS
/*
  View: CustomerService.agent_productivity_new_daily
  Notas:
  - Target diario NO viene de CustomerService.agent, sino de CustomerService.agent_target
    aplicando reglas por department_id + rango de months_of_service (min/max).
  - Si hire_date está ausente, months_of_service=0 como fallback.
  - Usa DECIMAL amplio para evitar overflow (Error 8115).
  - outbound_total desde CustomerService.outbounds con COUNT(DISTINCT outbound_id).
  - achieved_pct devuelve ratio en [0,1] (NO multiplicado x100).
  - Los casos transferidos SÍ cuentan en la productividad en ambos modelos.
*/
...
vonage AS (
  /* Métricas Vonage agregadas por agente/día. */
  SELECT
...
resolved AS (
  /* Casos resueltos por agente/día. */
  SELECT
...
transferred AS (
  /* Casos transferidos por agente/día. */
  SELECT
...
outbounds AS (
  /* Acciones outbound por agente/día usando DISTINCTCOUNT(outbound_id). */
  SELECT
...
SELECT
  ...
  /* Claves y nombre del agente */
  a.agent_id                                     AS agent_id,
  CONCAT(a.first_name, ' ', a.last_name)         AS agent_name,

  /* Claves y nombre del departamento (via agent.department_id) */
  dep.department_id                              AS department_id,
  dep.department_name                            AS department_name,

  /* Target diario del agente (desde agent_target por dept + months_of_service) */
  COALESCE(tgt.item_target_day, 0)               AS item_target_day,

  /* Métricas de tiempo Vonage */
  ...
  /* Outbounds total (columna ya existente) */
  COALESCE(ob.outbound_total, 0)                 AS outbound_total,

  /* Flag: departamentos que NO usan Vonage (incluye 3) */
  CASE
  ...
  /* Target del modelo (ajustado para si-Vonage; fijo para no-Vonage) */
  at.adjusted_target_calc                        AS target_model,

  /* Productividad del modelo (resueltos + transferidos) */
  CASE
    WHEN dep.department_id IN (2,3,12,13,14,15,16,18,21) THEN
      -- no-Vonage: cuentan todos los casos del día
      CAST(COALESCE(rs.resolved_total, 0) + COALESCE(tf.transfer_total, 0) AS DECIMAL(19,4))
    ELSE
      -- si-Vonage: cuentan sólo si hubo tiempo logado ese día
      CASE
      ...
      END
  END AS prod_total_model,

  /* % de logro del target (ratio 0–1; puede superar 1 si sobrecumple) */
  ca.achieved_pct
...
/* ------------------------------------------------------------------
   OUTER APPLY: calcular months_of_service y elegir target aplicable
   ------------------------------------------------------------------ */
OUTER APPLY (
  SELECT
    /* Si no hay hire_date, cae a 0 meses para tomar el tramo base */
    CASE
...
OUTER APPLY (
  /* Selecciona el target cuyo rango [min_months, max_months] cubre months_of_service
     y coincide el department_id del agente. Si hay varias filas, toma la más específica
     (por ejemplo, la de mayor min_months). */
  SELECT TOP (1)
         t.item_target_day
  FROM CustomerService.agent_target AS t
...
/* Arithmetic controlado: tipos DECIMAL y división segura con NULLIF */
CROSS APPLY (
  SELECT
    CASE
...
CROSS APPLY (
  SELECT
    /* target_model: fijo para no-Vonage, ajustado por ratio para si-Vonage */
    CAST(
      CASE
        WHEN dep.department_id IN (2,3,12,13,14,15,16,18,21) THEN
  ...
  SELECT
    CAST(
      CASE
        ...
        ELSE NULL
      END
      AS DECIMAL(12,6) -- ratio en [0,1] (puede exceder 1 si sobrecumple)
) AS achieved_pct
) AS ca;
